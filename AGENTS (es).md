...
> **Objetivo**: Mantener estándares de calidad de código 2026. Alta cohesión, bajo acoplamiento, deuda técnica cero.
...
├── core/           # Contenedor DI, Event Bus
├── domain/         # Entidades, Puertos (Interfaces), Errores
├── application/    # Command Handlers (Casos de Uso)
└── infrastructure/ # Implementaciones Concretas (Whisper, SoundDevice)
```

### Reglas
1.  **Interfaces definidas en Dominio**: `infrastructure` las implementa. `application` las usa.
2.  **No "God Classes"**: Divide responsabilidades (ej. `AudioRecorder` vs `TranscriptionService`).
3.  **Type Hints**: 100% cobertura requerida. Usa `typing.Protocol` para interfaces.
4.  **AsyncIO**: El núcleo es async. No bloquees el event loop con cómputo pesado.
...
- **Rutas Hardcodeadas**: NUNCA uses rutas absolutas como `/home/user`. Usa `pathlib` relativo a la raíz o config.
...
- Usa manejo de errores estilo **Rust** en Python donde sea posible.
