// Voice2Machine - Puente IPC Tauri
...
/// Ruta del socket: lee de la variable de entorno V2M_SOCKET_PATH o descubre dinámicamente XDG_RUNTIME_DIR.
...
fn socket_path() -> &'static str {
    ...
    PATH.get_or_init(|| {
        ...
        let mut path = if let Ok(xdg) = env::var("XDG_RUNTIME_DIR") {
            ...
            // Fallback a /tmp con el UID del usuario (seguro en Linux si se verifica la propiedad)
        ...
        // Esto previene ataques de "squatting" donde otro usuario crea el directorio primero.
        if path.exists() {
            use std::os::unix::fs::MetadataExt;
            if let Ok(metadata) = std::fs::metadata(&path) {
                if metadata.uid() != uid {
                    // Fallo crítico de seguridad: Directorio propiedad de otro usuario.
                    // Fallback a un nombre temporal aleatorio o fallar ruidosamente.
                    eprintln!("ERROR DE SEGURIDAD: El directorio de ejecución {:?} no es propiedad del UID {}", path, uid);
                }
            }
        } else {
...
/// Respuesta cruda del daemon (interno)
#[derive(Deserialize)]
...
/// Estado tipado del daemon (enviado al frontend)
...
/// Envía una petición JSON al daemon Python vía socket Unix.
/// Retorna un Value tipado en caso de éxito, o IpcError en caso de fallo.
fn send_json_request(command: &str, data: Option<Value>) -> Result<Value, IpcError> {
    // 1. Conectar al socket
    let mut stream = UnixStream::connect(socket_path())
        .map_err(|e| IpcError::from(format!("Daemon no está corriendo: {}", e)))?;

    stream
        .set_read_timeout(Some(std::time::Duration::from_secs(READ_TIMEOUT_SECS)))
        .map_err(|e| IpcError::from(format!("Error config timeout: {}", e)))?;
    ...
    if response.status == "success" {
        ...
        Err(IpcError {
            ...
            message: response.error.unwrap_or_else(|| "Error desconocido del daemon".to_string()),
        })
    }
}
...
/// Obtener estado actual del daemon con telemetría
#[tauri::command]
fn get_status() -> Result<DaemonState, IpcError> {
    let data = send_json_request("GET_STATUS", None)?;
    serde_json::from_value(data).map_err(|e| IpcError::from(format!("Error parseo: {}", e)))
}
...
fn start_recording(app: tauri::AppHandle) -> Result<DaemonState, IpcError> {
    ...
    // Emitir evento de cambio de estado a todos los listeners
...
fn stop_recording(app: tauri::AppHandle) -> Result<DaemonState, IpcError> {
    ...
    let state: DaemonState = serde_json::from_value(data)
        .map_err(|e| IpcError::from(format!("Error parseo: {}", e)))?;
    // Emitir resultado de transcripción
    let _ = app.emit("v2m://state-update", &state);
    Ok(state)
}

/// Ping daemon (chequeo de salud)
#[tauri::command]
...
/// Reanudar operaciones del daemon
#[tauri::command]
...
/// Helper: Resolver ruta al script del daemon.
fn resolve_daemon_script(app: &tauri::AppHandle) -> Result<std::path::PathBuf, IpcError> {
    ...
    if let Some(path) = dev_path.filter(|p| p.exists()) {
        ...
        app.path()
            ...
            .map_err(|e| IpcError::from(format!("Error resolviendo ruta script: {}", e)))
    }
}

/// Reiniciar proceso del daemon
#[tauri::command]
async fn restart_daemon(app: tauri::AppHandle) -> Result<DaemonState, IpcError> {
    ...
    let output = SysCommand::new("bash")
        ...
        .arg("restart")
        .output()
        .map_err(|e| IpcError::from(format!("Error script reinicio: {}", e)))?;
    ...
    if output.status.success() {
        let state = DaemonState {
            ...
            transcription: None,
            refined_text: None,
            message: Some("Daemon reiniciado".to_string()),
            telemetry: None,
        };
        let _ = app.emit("v2m://state-update", &state);
        Ok(state)
...
async fn shutdown_daemon(app: tauri::AppHandle) -> Result<DaemonState, IpcError> {
    ...
    let output = SysCommand::new("bash")
        ...
        .arg("stop")
        .output()
        .map_err(|e| IpcError::from(format!("Error script apagado: {}", e)))?;
    ...
    if output.status.success() {
        let state = DaemonState {
            ...
            transcription: None,
            refined_text: None,
            message: Some("Daemon detenido".to_string()),
            telemetry: None,
        };
        let _ = app.emit("v2m://state-update", &state);
        Ok(state)
